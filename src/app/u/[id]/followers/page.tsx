...
export default async function PublicFollowersPage(context: Props) {
  const { id } = await context.params

  // If current user is the same, redirect to private followers page
  try {
    const supabase = await getServerSupabase()
    const { data } = await supabase.auth.getSession()
    const meId = data.session?.user?.id || null
    if (meId && meId === id) redirect('/followers')
  } catch {}
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="text-lg font-semibold text-gray-900">@{user.username || String(user.id).slice(0,6)} Followers</div>
        <div className="w-5" />
      </div>
      ...
      {followers.length ? (
        ...
        <div className="text-center text-gray-600">Takipçi yok.</div>
      )}
    </div>
  )
}
