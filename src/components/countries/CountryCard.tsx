...
export function CountryCard({ country, className }: CountryCardProps) {
  ...
  const getLocalized = (i18nField: any, fallback: string | null | undefined) => {
    if (!i18nField || typeof i18nField !== 'object') return fallback || ''
    return i18nField?.[locale as string] || i18nField?.en || Object.values(i18nField).find((v: any) => v) || fallback || ''
  }
      ...
      <Card className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all">
        ...
        <div className="p-4">
          ...
          <p className="mt-3 text-sm text-gray-600">
            {getLocalized((country as any).culture_description_i18n, country.culture_description) || country.food_description || t('card.fallbackDescription')}
          </p>
          ...
          <div className="mt-3 grid grid-cols-3 gap-2 text-[11px] text-gray-600">
            <div className="inline-flex items-center gap-1">
              ...
              <span className="truncate">{getLocalized((country as any).best_time_to_visit_i18n, country.best_time_to_visit) || t('card.allYear')}</span>
            </div>
            <div className="inline-flex items-center gap-1">
              <Thermometer className="h-3.5 w-3.5 text-gray-400" />
              <span className="truncate">{getLocalized((country as any).climate_info_i18n, country.climate_info) || t('card.temperate')}</span>
            </div>
            <div className="inline-flex items-center gap-1">
